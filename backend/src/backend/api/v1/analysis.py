...
def _get_official_corners(session_key: int) -> list | None:
    ...
    Only caches None on hard failures (missing session), not on transient errors.
    ...
    try:
        ...
        with engine.connect() as conn:
            sess_row = (
                conn.execute(
                    text("SELECT year, gp_name, session_type FROM sessions WHERE session_key = :sk"),
                    {"sk": session_key},
                )
                .mappings()
                .first()
        ...
        if not sess_row:
            # Hard failure: session doesn't exist, cache None
        ...
        # Use the actual session type instead of always forcing 'Q'
        session_type = sess_row["session_type"] or "Q"
        gp_name = sess_row["gp_name"]
        
        # Remove common suffixes that FastF1 doesn't recognize
        for suffix in [" Grand Prix", " GP"]:
            if gp_name.endswith(suffix):
                gp_name = gp_name[:-len(suffix)]
        
        ff1_session = ff1.get_session(
            int(sess_row["year"]), gp_name, session_type
        )
        ...
        # Transient error (network, data loading): don't cache None
        # Log the error and return None without caching
        import logging
        log = logging.getLogger(__name__)
        log.warning(f"Failed to load corners for session {session_key}: {e}")
        return None
