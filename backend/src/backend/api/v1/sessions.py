...
def driver_standings():
    """
    Official driver championship standings via Jolpica-F1 API (direct).
    FastF1's Ergast wrapper was removed in 3.4+, so we call Jolpica directly.
    ...
    try:
        ...
        latest_round_data = standings_lists[-1]  # most recent round
        ...
        return jsonify({"error": "Failed to fetch standings from Jolpica API", "detail": str(e)}), 503
    except Exception as e:
        return jsonify({"error": "Unexpected error processing standings", "detail": str(e)}), 500


@sessions_bp.get("/standings/constructors")
def constructor_standings():
    """
    Official constructor championship standings via Jolpica-F1 API (direct).
