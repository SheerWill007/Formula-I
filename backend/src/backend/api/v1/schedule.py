...
def _load_results(year: int) -> dict[int, list[str]]:
    """Fetch Top 3 finishers for all rounds of the season via Jolpica API."""
    log.info(f"Loading {year} race results from Jolpica API...")
    try:
        import requests
        url = f"http://api.jolpi.ca/ergast/f1/{year}/results/1.json?limit=1000"
        ...
        round_results = {}
        for race in races:
            r_num = int(race.get("round", 0))
            results = race.get("Results", [])[:3]
            round_results[r_num] = [r.get("Driver", {}).get("code", "???") for r in results]
        
        log.info(f"Loaded results for {len(round_results)} rounds")
        return round_results
    except Exception as e:
        log.warning(f"Failed to load race results from Jolpica: {e}")
        return {}
